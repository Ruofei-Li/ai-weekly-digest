...
本项目最终支持**两套运行方案**，读者可根据自身条件选择：

---

#### 方案 A：GitHub Actions + Gmail（推荐）

适用于：有 Gmail 账号、希望全云端运行、不依赖本地机器。

```
┌─ 调度层 ──────────────────────────────────┐
│  GitHub Actions cron：每周一 9:00 (UTC+8)  │  零本地依赖
│  .github/workflows/weekly-digest.yml      │  全自动
└────────────────────┬──────────────────────┘
                     ▼
┌─ 数据采集层 ─────────────────────────────┐
...
关键配置：只需在 GitHub 仓库设置 Secrets 填入 Gmail App Password，无需任何本地环境。

---

#### 方案 B：Mac launchd + QQ 邮箱

适用于：没有国际邮件服务、希望本地运行、Mac 常开机/不在意休眠补偿。

```
┌─ 调度层 ──────────────────────────────────┐
│  Mac launchd：每 5 分钟轮询                │  依赖本地机器
│  run-digest.sh：三层过滤（周一/9点/锁）    │  休眠唤醒自动补发
└────────────────────┬──────────────────────┘
                     ▼
┌─ 数据采集层 ─────────────────────────────┐
│  同上（与方案 A 共用同一份代码）           │
...
└────────────────────┬──────────────────────┘
                     ▼
┌─ 发送层 ──────────────────────────────────┐
│  SMTP (QQ邮箱) → smtp.qq.com:587          │
│  → your_email@company.com                 │
└───────────────────────────────────────────┘
```

关键配置：本地 `.env` 文件填写 QQ 邮箱 SMTP 授权码，配合 launchd 实现定时触发。

#### 方案对比

| 维度 | 方案 A：Actions + Gmail | 方案 B：launchd + QQ |
|------|------------------------|---------------------|
| **邮件服务** | Gmail（全球可访问） | QQ 邮箱（国内可用） |
| **运行位置** | GitHub 云端服务器 | 本地 Mac |
| **是否需要本地开机** | ❌ 不需要 | ✅ 需要（休眠可补发） |
| **配置复杂度** | 低（只有 Secrets） | 中（需配 launchd plist） |
| **适用场景** | 有 Gmail 账号、想全托管 | 国内网络、无国际邮箱 |
| **数据采集节点** | 美国（GitHub） | 中国（本地） |
...
原本以为最容易的部分，结果成了最折腾的。这个故事实际上贯穿了我们从方案 A 到方案 B 再到方案 A+B 的全过程。
...
**第五回合：转回本地，用 Mac launchd 绕过网络限制**

云端发不通，那就本地跑。用 macOS 自带的 `launchd` 替代 cron，实现定时触发。

**第六回合：最终用 Gmail 打通云端**

最后尝试 Gmail SMTP + App Password，GitHub Actions 一次跑通。全球可访问，配置简单，全云端运行目标达成。

> 最终状态：**两套方案并行**——GitHub Actions + Gmail 做主力，Mac launchd + QQ 邮箱做本地备用。

---

#### launchd 实现方案详解（适用于方案 B）
...
> **教训**：GitHub Actions 虽然是优秀的 CI/CD 工具，但涉及网络请求到特定区域的服务时（特别是中国国内的服务），网络连通性是要提前考虑的约束条件。最终我们提供了两条路径——有 Gmail 走云端，没有就本地 launchd + QQ 邮箱，各取所需。
...
```
ai-weekly-digest/
├── .github/workflows/weekly-digest.yml    # GitHub Actions 调度
├── run-digest.sh                           # launchd 调度脚本（方案 B）
├── .env.example                            # 配置模板
├── requirements.txt                        # 依赖
├── README.md                               # 使用文档
...
**设计考量**：不用 YAML/JSON 配置文件，而是用环境变量 + `os.getenv()`。这样做的好处是：和 GitHub Secrets 天然对接，本地用 `.env` 文件（`python-dotenv` 加载），敏感信息不会被提交到仓库。无论读者选择方案 A 还是方案 B，配置方式完全一致，只是填入的值不同。
...
关键设计：支持 SSL（465）和 STARTTLS（587）两种模式。方案 A 用 Gmail 587，方案 B 用 QQ 邮箱 587，共用同一份代码。

### run-digest.sh——本地调度脚本（方案 B 专属）
...
| **方案 A** | GitHub Actions cron → Gmail SMTP（全云端） |
| **方案 B** | Mac launchd 每 5 分钟 → QQ 邮箱 SMTP（本地） |
...
1. **方案没有最好，只有最适合**。一开始执着于"全部上云"，觉得 GitHub Actions 自动化才是正解。但卡在国内网络问题上后，发现本地 launchd 也很靠谱。最终两条路径都跑通了——有 Gmail 就全云端，没有就本地跑，代码完全一样，只是配置不同。**多一条退路，少一分焦虑。**
...
6. **技术选型要考虑网络拓扑**。GitHub Actions 节点在美国，QQ 邮箱在国内——这个简单的网络事实成了整个项目最大的坑。选型之前，先画一张网络连通性图，能省很多试错时间。
